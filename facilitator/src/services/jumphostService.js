...
const attemptHistoryService = require('./attemptHistoryService');
...
async function evaluateExamOnJumphost(examId, questions) {
  try {
    ...
    try {
      ...
      if (examInfo) {
        await attemptHistoryService.appendAttempt(examId, examInfo, finalResult);
      } else {
        logger.warn(`Skipping attempt history because exam info was not found for exam ${examId}`);
      }
    } catch (historyError) {
      logger.error('Failed to persist attempt history', {
        examId,
