...
/**
 * Get persisted exam attempt history
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 */
async function getExamAttempts(req, res) {
  logger.info('Received request to get exam attempts', { filters: req.query });

  try {
    const result = await attemptHistoryService.getAttempts({
      category: req.query.category,
      labId: req.query.labId,
      labName: req.query.labName,
      failedConcept: req.query.failedConcept,
      from: req.query.from,
      to: req.query.to
    });
    ...
    logger.error('Error retrieving exam attempts', { error: error.message });
    return res.status(500).json({
      success: false,
      error: 'Failed to retrieve exam attempts',
      message: error.message
    });
  }
}
