...
This skill is repository-native for Codex. When the user asks to generate a lab, create or update files directly in the repository after inspecting the current CK-X structure and confirming only when an overwrite or risky decision is involved.
...
Before creating or changing a lab, Codex must inspect the repository.
...
1. Confirm the CK-X repository root.
   ...
   - If Codex is not running from the repository root, stop and ask the user to reopen from the CK-X root.
...
```bash
.agents/skills/cka-exam-generator-2026/scripts/validate_ckx_lab.sh facilitator/assets/exams/cka/NNN
```
